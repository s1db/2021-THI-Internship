...
class CopelandRunner:
    ...
    def run_basic(self):
        ...
        self.create_debug_folder("basic")
    ...
    def run_extended(self):
        ...
        self.create_debug_folder("extended")
        # we'll need a solution pool of previously seen solutions
        # to rule out condorcet cycles; a solution is stored as a Python dictionary from variable to value
        solution_pool = []
    ...
    def create_debug_folder(self, folder):

        self.debug_dir = ("debug" + "/" + folder)
    ...
    def scores_calc(self, a, score):
        return 2*np.sum(score[a,:] > VOTERS/2) + np.count_nonzero(score[a,:] == VOTERS/2)

    def final_scores(self, score):
        return np.fromiter((self.scores_calc(x, score) for x in range(CANDIDATES)), int)